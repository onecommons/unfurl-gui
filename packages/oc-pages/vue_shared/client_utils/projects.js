...
/*
 * What is queued against each base commit on a branch -- the fact `/branches`
 * cannot report, knowing only where the repo is.
 *
 * Fail-soft: null on any failure, since no read should fail for this. A server
 * with no proxy answers `{commits: {}}`, which is true rather than a stub:
 * with no queue, nothing is ever pending.
 */
...
export async function fetchLastCommit(projectPath, _branch) {
    ...
    // Only meaningful against a commit `/branches` just fetched, so it rides
    // with that rather than firing on every read -- which also spares a server
    // with no queue a round trip per read.
    const branchesWereCached = Boolean(branchesData[projectId])
    const [branches, queued] = await Promise.all([
        fetchBranches(projectId),
        branchesWereCached ? null : fetchQueueState(projectPath, branch),
    ])
